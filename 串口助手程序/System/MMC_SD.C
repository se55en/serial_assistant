...
u8 SD_SendCmd(u8 cmd, u32 arg, u8 crc)
{
	...
	//发送
    SD_SPI_ReadWriteByte(cmd | 0x40);//分别写入命令，最高两位是01，命令在后六位
...
u8 SD_Initialize(void)
{
	...
	SD_SPI_Init();		//初始化IO
 	...
 	for(i=0;i<10;i++)SD_SPI_ReadWriteByte(0XFF);//发送最少74个脉冲，CLK延时>74个，一个数据8个，一共80个脉冲
	retry=20;
	do
	{
//		myLCD_ShowString(X[10], Y[10], "111    ");
